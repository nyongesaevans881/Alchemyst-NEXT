...
export default function Navbar() {
  ...
  // Initialize from localStorage and set up listeners on mount
  useEffect(() => {
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {token && user ? (
                ...
                {dropdownOpen && (
                  ...
                  >
                    ...
                    <div className="flex flex-col text-sm text-text-primary">
                      <button onClick={() => { router.push("/dashboard"); setDropdownOpen(false); }} className="px-4 py-2 text-left hover:bg-primary cursor-pointer hover:text-white">Dashboard</button>
                      <button onClick={() => { router.push("/dashboard#settings"); setDropdownOpen(false); }} className="px-4 py-2 text-left hover:bg-primary cursor-pointer hover:text-white">Settings</button>
            ...
            <div className="flex items-center gap-3">
              <Link href="/login" className="px-6 py-2.5 text-text-inverse border border-primary rounded-lg font-medium hover:bg-primary/10 transition-all">Login</Link>
              <Link href="/register" className="px-6 py-2.5 bg-primary text-text-inverse rounded-lg font-medium hover:bg-primary-hover transition-all hover:scale-105">Sign Up</Link>
            </div>
          )}
        </div>
            ...
            <div className="flex flex-col items-center gap-4 w-full px-8">
              ...
              {token && user ? (
                <div className="flex flex-col items-center gap-4 text-center">
                  ...
                  <button onClick={() => { router.push("/dashboard"); setMobileMenuOpen(false); }} className="text-2xl font-bold text-text-inverse hover:text-primary">Dashboard</button>
                  <button onClick={() => { router.push("/settings"); setMobileMenuOpen(false); }} className="text-2xl font-bold text-text-inverse hover:text-primary">Settings</button>
                  <button onClick={handleLogout} className="text-2xl font-bold text-red-400 hover:text-red-300">Logout</button>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-4 w-full max-w-xs">
                  <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="w-full px-8 py-4 text-text-inverse border-2 border-primary rounded-lg font-bold text-xl hover:bg-primary/10 transition-all text-center">Login</Link>
                  <Link href="/register" onClick={() => setMobileMenuOpen(false)} className="w-full px-8 py-4 bg-primary text-text-inverse rounded-lg font-bold text-xl hover:bg-primary-hover transition-all text-center">Sign Up</Link>
                </div>
              )}
            </div>
          </motion.div>
