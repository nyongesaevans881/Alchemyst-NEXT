...
export default function MasseusesPage() {
  ...
  // Set userType to masseuse on mount
  useEffect(() => {
    if (filters.userType !== 'masseuse') {
      updateFilters({ ...filters, userType: 'masseuse' })
    }
  }, [])
  ...
  const handleSearchChange = (value) => {
    ...
    const serviceMatches = [
      "Massage", "Dinner Dates", "Travel Companion", "GFE", "PSE", "Couples",
      "Sleepovers", "Threesomes", "Role Play", "Lesbian Shows", "Anal", "BDSM",
      "BJ", "Raw BJ", "Rimming", "Raw Sex", "Golden Shower", "Sex Toys"
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    if (suggestion.type === "county") {
      const path = generateSeoPath({ county: suggestion.county })
      router.push(path)
      ...
      updateFilters({ ...filters, specificService: suggestion.service })
    } else if (suggestion.type === "bodyType") {
      updateFilters({ ...filters, bodyType: suggestion.bodyType })
    }
  }
        ...
        <div className="relative container mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-4">
            <h1 className="text-4xl md:text-5xl font-bold text-text-inverse mb-4 max-md:flex max-md:flex-col">
                Alchemyst
              <span className="bg-[url('/graphic/scratch.png')] bg-cover bg-center bg-no-repeat py-2 px-2"> masseuses </span>
            </h1>
          </motion.div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              <span>{displayedCount} masseuses found</span>
              {lastFetchTime && (
                <>
                  <span>•</span>
