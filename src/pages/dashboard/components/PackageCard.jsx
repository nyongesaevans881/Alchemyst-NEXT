...
export default function PackageCard({ userData, updateUserData }) {
  ...
  const [autoRenewDuration, setAutoRenewDuration] = useState("weekly")

  const userPackages = PACKAGE_TIERS[userData?.userType] || PACKAGE_TIERS.escort
  ...
  // Get package details
  const currentPackageDetails = currentPackage ? userPackages[currentPackage.packageType] : null

  const calculatePrice = (weeklyPrice) => {
  ...
  // Check if can upgrade to this package
  const canUpgrade = (pkg) => {
    if (!currentPackage || !isActive) return false
    const packagePriority = { 'basic': 1, 'premium': 2, 'elite': 3 }
  ...
  const handleAutoRenew = async (enabled) => {
    setLoading(true)
    try {
  ...
  const handleCancelPackage = async () => {
    if (!confirm("Are you sure you want to cancel? Your package will remain active until expiry.")) {
      return
    }
        ...
        {isActive && currentPackageDetails ? (
          <div className="space-y-4">
            ...
            <div className={`border rounded-xl p-4 ${
              needsRenewal ? 'bg-warning/10 border-warning/30' : 
              isCancelled ? 'bg-error/10 border-error/30' :
              'bg-success/10 border-success/30'
            }`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  {currentPackageDetails.icon && (
                    <currentPackageDetails.icon className="text-primary text-2xl" />
                  )}
                  <div>
                    <h4 className="font-bold text-text-primary">{currentPackageDetails.name}</h4>
                    <p className="text-xs text-text-muted capitalize">
                      {currentPackage.durationType} plan
                    </p>
                  </div>
                </div>
                <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                  isCancelled ? 'bg-error/20 text-error' :
                  needsRenewal ? 'bg-warning/20 text-warning' : 
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="bg-bg-primary/50 rounded-lg p-2">
                  ...
                  <div className="text-xl font-bold text-primary">
                    {calculateRemainingDays(currentPackage.expiryDate)}
                  </div>
                </div>
                <div className="bg-bg-primary/50 rounded-lg p-2">
                  <div className="flex items-center gap-1 text-text-muted text-xs mb-1">
                  ...
                  <div className="text-sm font-bold text-text-primary">
                    KSh {currentPackage.totalCost?.toLocaleString() || 0}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
              ...
              {currentPackage.packageType !== 'elite' && !isCancelled && (
                <button
                  onClick={() => {
                    const nextPackage = Object.values(userPackages).find(
                      pkg => canUpgrade(pkg)
                    )
            ...
            <div className="bg-bg-primary rounded-lg p-4">
              ...
              <ul className="space-y-1.5">
                {currentPackageDetails.features.map((feature, index) => (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 overflow-y-auto">
            ...
            >
              <h3 className="text-2xl font-bold text-text-primary mb-2">
                {actionType === 'subscribe' ? 'Choose Your Package' :
                 actionType === 'renew' ? 'Renew Your Package' : 
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-center mb-4">
                            ...
                            <p className="text-text-muted text-sm">
                              {durationType === "weekly" ? "per week" : "per month"}
                            </p>
                            {!canAfford && (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-bold text-text-primary">
                        {actionType === 'subscribe' ? 'New Subscription' :
                         actionType === 'renew' ? 'Renewal' : 'Upgrade'}
                      </h4>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-bold text-primary">
                        KSh {calculatePrice(selectedPackage.weeklyPrice).toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {loading ? (
                    ...
                    `Confirm ${actionType === 'subscribe' ? 'Subscription' : 
                      actionType === 'renew' ? 'Renewal' : 'Upgrade'}${
                ...
                <>
                  ...
                  <div className="bg-warning/10 border border-warning/30 rounded-lg p-3 mb-4">
                    <p className="text-xs text-text-muted">
                      <strong>Note:</strong> Ensure sufficient wallet balance before expiry. 
                      Auto-renew will fail if balance is insufficient.
                    </p>
                  </div>
                </>
              ) : (
