...
export default function LocationCard({ userData, updateUserData }) {
  ...
  const MAX_AREAS = isActive
    ? activePackage.packageType === "premium" || activePackage.packageType === "elite"
      ? 5
      : 3
    : 3;
  ...
  useEffect(() => {
    ...
    const allFieldsFilled = requiredFields.every(field =>
      Boolean(userData?.location?.[field])
    );
  ...
  // Load sub-counties dynamically when county changes
  useEffect(() => {
    if (formData.county) {
  ...
  return (
    <motion.div
      ...
      className={`bg-bg-secondary border border-border-light rounded-2xl p-6 ${isLocationComplete ? 'border-green-500' : 'border-border-light'}`}
    >
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-text-primary text-sm">
                  {formData.country ? <span className="font-bold text-green-600">{formData.country}</span> : <span>Not set</span>}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-muted text-sm">County:</span>
                <span className="text-text-primary text-sm">
                  {formData.country ? <span className="font-bold text-green-600">{formData.county}</span> : <span>Not set</span>}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-muted text-sm">location:</span>
                <span className="text-text-primary text-sm">
                  {formData.country ? <span className="font-bold text-green-600">{formData.location}</span> : <span>Not set</span>}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="text-text-primary text-sm">
                  {formData.area.length > 0 ? <span className="font-bold text-green-600">{formData.area.join(", ")}</span> : <span>Not set</span>}
                </span>
              </div>
            </div>
            <button
