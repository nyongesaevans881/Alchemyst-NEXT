...
export default function ProfileVerificationCard({ userData, updateUserData }) {
  const [loading, setLoading] = useState(false)

  const handleRequestVerification = async () => {
    // Check if user has premium or elite package
    const hasPremiumPackage = userData?.currentPackage?.packageType === 'premium' || userData?.currentPackage?.packageType === 'elite';
  ...
  // Update the condition checks at the top:
    const hasPremiumPackage = userData?.currentPackage?.packageType === 'premium' || userData?.currentPackage?.packageType === 'elite';
  ...
  return (
    <motion.div
      ...
      className={`bg-bg-secondary border rounded-2xl p-6 ${verificationStatus === 'pending' ? "border-yellow-400" : profileVerified ? "border-green-400" : "border-border-light"}`}
    >
